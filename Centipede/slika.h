...
public:
    Slika();
    Slika(char* filename);
    Slika(int numRows, int numCols); // Initialize a blank bitmap of this size.
    ~Slika();

    bool LoadBmpFile(char* filename);       // Loads the bitmap from the specified file
...
inline Slika::Slika(char* filename)
{
    NumRows = 0;
    NumCols = 0;
    ImagePtr = 0;
...
inline const unsigned char* Slika::GetRgbPixel(long row, long col) const
{
...
inline unsigned char* Slika::GetRgbPixel(long row, long col)
{
    assert(row < NumRows && col < NumCols);
...
inline void Slika::GetRgbPixel(long row, long col, float* red, float* green, float* blue) const
{
    assert(row < NumRows && col < NumCols);
...
inline void Slika::GetRgbPixel(long row, long col, double* red, double* green, double* blue) const
{
    assert(row < NumRows && col < NumCols);
    const unsigned char* thePixel = GetRgbPixel(row, col);
